...
The Hebb's rule defines the couplings as:
...
The next step is to define a sort of dynamics that make the network able to retrieve a stored pattern from a slightly different one.
...
\begin{equation}\label{eq:prob_configuration}
    Z = \sum_{\symbf{\sigma}} exp\{- \beta E(\symbf{\sigma}) \},
\end{equation}
...
\end{equation}
Moreover, we introduce the \emph{load parameter}, \ie:
\begin{equation}
    \alpha = \frac{M}{N},
\end{equation}
...
Results show that there exist a critical value of the load parameter $\alpha_c = 0.138$ above which retrieval states disappear discontinuously. Here, the system is on a spin glass phase. Below $\alpha_c$ patterns can be retrieved at $T = 0$ with an error smaller than $1.5\%$. Moreover, for $\alpha \leq 0.05$ patterns become global minimum. For $0.05 < \alpha < \alpha_c$ patterns are metastable.
...
\subsection{Phase Diagram}
We do not have a phase diagram!!!
