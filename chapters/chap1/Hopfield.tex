...
\end{equation*}
then stability is destroyed~\cite{hopfield_capacity}. However~\citeauthor{JJHop} pointed out that the \acrlong{am} behaviour can be generalized if one allows the presence of a small fraction of misaligned spins between the retrieved state and the original memory. In this case storage capacity is linear,
...
\begin{equation}\label{eq:order_parameters}
    m^\mu = \frac{1}{N}\sum_{i = 1}^N \langle \sigma_i \rangle \xi_i^\mu,
\end{equation}
...
The \acrshort{shm} shows an interesting phase diagram on the $T$-$\alpha$ plane which has been investigated first by~\citeauthor{amit_phase}.\\
...
\begin{itemize}
    \item Mattis-states or \emph{retrieval states} \textbf{è giusto???? sono la stessa cosa no?}, \ie $2M$ degenerate states each of which is correlated with only one stored memory;
...
\begin{enumerate}
    ...
    \item spurious states are well separated by energiy barriers \textbf{dubbio}\footnote{mettere una footnote dove dico qualcosa in più}.  
\end{enumerate}
...
\begin{equation}\label{eq:orthogonality}
    m^{\mu \nu} \equiv \frac{1}{N} \sum_{i = 1}^{N} \xi_i^\mu \xi_j^\nu = \delta_{\mu \nu} +  O\left(1/\sqrt{N} \right) \left(1 - \delta_{\mu \nu} \right).
\end{equation}
...
\end{equation}
\textbf{dire qualcosina in più qui su come si va avanti}
...
The core of the memory-behaviour of the \acrlong{hm} (for both the standard and modern versions) relies on the fact that stored patterns are attractors for a low-temperature dynamics (\textbf{è corretto dire così?}). All the starting configurations that evolve towards the same attractor define what is called a \acrfull{ba}. The structure of such basins is quite hard to investigate, and it gets more and more complicated as the number of stored memories increases~\cite{Keeler}. One of the main goals of this work is to measure ``how large'' such basins are, in terms of the noise applied to a given memory. We will provide more details on this numerical results for this in~\cref{chap:radius}. However, we introduce now some notions that will be useful for the next chapters.\\
First a natural distance measure that can be used in this case is the Hamming distance.
\begin{definition}\label{def:hamming_distance}
Given two binary $N$-dimensional configurations $\bsigma^mu$ and $\bsigma^\nu$ the Hamming distance between the two is just the number of neurons that differ. In mathematical form, we call $\mathcal{A}$ the set that contains the positions where $\bsigma^\mu$ is different from $\bsigma^\nu$:
\begin{equation*}
    \mathcal{A} = \left\{ 1 \leq i \leq N \mid \sigma_i^\mu \neq \sigma_i^\nu  \right\},
\end{equation*}
hence, the Hamming distance is just
\begin{equation}
    d\left(\bsigma^\mu, \bsigma^\nu \right) =  |A|
\end{equation}
and it is indeed a metric.
\end{definition}
As pointed out by~\textcite{Kanter} the key condition that allows the presence of retrieval states starting from noisy configurations, is the presence of the \acrlong{ba} in addition to the stability of stored memories. The consequence is that below a certain $\alpha_c$, whether this refers to the \acrshort{shm} or to the \acrshort{mhm}, such basins can be characterized by a suitable average distance, which, in principle, is null above $\alpha_c$. In~\cref{chap:phase_diagram} we exploit this fact by looking for the $\alpha$ value for which a deterministic dynamics escape from one of the learnt patterns. In other words, while for $\alpha < \alpha_c$ (almost) any configuration which has a macroscopic overlap, \ie much bigger than $O\left(1/\sqrt{N} \right)$, with only one memory is attracted by the latter, for $\alpha \geq \alpha_c$ spurious states become predominant.\\
Instead, in~\cref{chap:radius} we give a rigorous definition of the size of the basins along with the corresponding numerical results.
\subbibliography
\end{document}