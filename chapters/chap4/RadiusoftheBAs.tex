...
\begin{figure}
    ...
    \caption{Example of scaling relationships for three different values of $\alpha$. Data are fitted with a polynomial of degree $2$ following~\cref{eq:scaling_noise}. Errorbars are given by error propagation of~\cref{eq:solution_fit}.}
...
Now, the goal is to investigate the space distribution of such basins. To this end, the idea is that, for a given \bxi\ we rank all the other patterns by measuring the overlap with \bxi. Doing so, we can perturb \bxi\ along the direction of the pattern with which it has the largest overlap.
Indeed, let's say that \bxi\ and \bxi[\nu]\ are the pattern that we want to perturb and the pattern with the largest overlap respectively. Then, the procedure consists on flipping with probability $p$ all the neurons that make $\bxi$ different from \bxi[\nu].
\begin{figure}
    \centering
...
\end{figure}
\Cref{fig:sh:radius:anisotropy} shows that the radius of the \acrlong{ba} is not isotropic. Surprisingly, we observe that basins are wider if we move along the direction of the most correlated \bxi, thus introducing some anisotropy in the geometry of the energy landscape.
It is not easy to justify this last result since, in principle, we would have expected to get a smaller radius. However, it is reasonable to assume that, along the most correlated direction, the energy landscape is smoother with respect to the \emph{random direction} case.
...
Nevertheless, the $\alpha_c$ for the \acrlong{mhm} is sligthly smaller that the one obtained in~\cref{chap:phase_diagrams} using the escape probability. This is probably due to the finite size effects that are not negligible in this case even if we are trying to extrapolate the thermodynamic limit. 
