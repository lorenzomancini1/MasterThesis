...
While for the \acrlong{shm} we have numerous and well known studies for its phase diagram~\cite{amit_phase}(qui forse la seconda citazione è da togliere), the same does not apply for the \acrlong{mhm} whose critical values, phase transitions and so on are still under investigation.\\
...
As we have discussed in~\cref{sec:modern_hop}, for the \acrshort{mhm}, stable configurations are determined by three parameters, which are the temperature $T$ (or its inverse $\beta$), the load parameter $\alpha$ and the extra parameters that appears in the energy computation $\lambda$.\\
...
If we are below the critical load parameter, $\alpha < \alpha_c$ and we run a low temperature \acrshort{mcmc} starting from one random stored pattern, we expect that the final configuration to be not to far from the initial one. ``Not to far'' means that the final overlap is as close as possible to the initial one, which is equal to unity if we start from a stored pattern. On the other hand, if we are in the region $\alpha > \alpha_c$ we should observe that the overlap of the final configuration is significantly different from the initial one, we say that the configuration \emph{escapes}.\\
What we are saying is that above $\alpha_c$ the stored patterns no longer correspond to local or global minima of the energy. In this case, stability is governed by either mixed or glassy states. Equivalently, \acrlong{ba} shrink more and more as $\alpha$ increases, until they disappear completely. More details on this will be given in the next chapter.\\
...
\begin{definition}[Escape probability]\label{def:pesc_definition}
Given an ensemble of random patterns matrices $\xi_{N \times \alpha N}$, the \emph{escape probability} is defined as:
\begin{equation}\label{eq:pesc_definition}
    \pesc \equiv \pescfull = \underset{\xi}{\mathbb{E}} \left[ \mathbb{I} \left( \frac{\bsigma_{t} \cdot \bxi}{N} \geq m_0 \right) \right], \qquad \mu = 1, \dots, \alpha \cdot N,
\end{equation}
where $\bsigma_t$ is the result -- at time $t$ -- of the evolution of the configuration \bxi,
\begin{equation*}
    \bxi = \bsigma_0 \overset{\text{\tiny\acrshort{mcmc}}}{\longrightarrow} \bsigma_t.
\end{equation*}
\end{definition}
...
\begin{algorithm}
    ...
    \label{alg:escape}
    \begin{algorithmic}[1]
    \Require $N$, $\alpha \alpha$, $\beta$, $\mathtt{nsamples}$,$\mathtt{nsweeps}$, $\mathtt{earlystop}$, $m_0$
    \State $M \gets N\alpha$ (or $\exp(N\alpha)$)
    \State $\mathtt{n\alpha} \gets |\alpha \alpha|$
    \State Initialize \texttt{freqs}, \texttt{ferrors} \Comment{arrays that contain probabilities and related errors}

    \For{$i = 1$ to $\mathtt{n \alpha}$}
        ...
        \EndFor
        \State Compute the number of \texttt{escapes} by checking $m_f < m_0$ elementwise
        \State $freqs[i] \gets$ mean(\texttt{escapes})
        \State $ferrors[i] \gets$ std(\texttt{escapes})
    \EndFor
    \State \textbf{return} \texttt{freqs}, \texttt{ferrors}
    \end{algorithmic}
\end{algorithm}
We expect \pesc\ to be something like a sigmoid for finite size systems, \ie it shows a smooth step -- from zero to one --  that covers a certain range of $\alpha$. Hence, simulation data should follow a function like:
\begin{equation}\label{eq:fit_pesc}
    \pesc= \frac{1}{1 + \exp \left[ - k_1 (\alpha - k_2) \right]} + k_3.
\end{equation}
...
\begin{equation}\label{eq:solution_fit}
    \alpha_c(N)_{y} = - \frac{\log\left(\left(y - k_3^*\right)^{-1} - 1\right)}{k_1^*} + k_2^*,
\end{equation}
...
\begin{equation}\label{eq:critical_alpha}
 ...
 \end{equation}
 The solution for different sizes given by~\cref{eq:solution_fit} can be plotted against the $1/N$ variable.
...
\begin{equation*}
    \alpha_c \gets q.
\end{equation*}
For all our simulations we set $y = 0.5$ in~\cref{eq:solution_fit}. Clearly, everyghing is valid for both the \acrshort{shm} and the \acrshort{mhm}, keeping in mind that, concerning the latter, the critical value depends on $\lambda$.\\
...
Before showing our results, it is worth to exploit~\cref{alg:escape} also for the \acrshort{shm} in order to have a comparison with theoretical results.\\
In~\cref{fig:sh:escaoe_probs} we can observe the sigmoid-like behaviour of the \pesc\ function, where the steepness increases with the size of the system, thus tending towards a sharp step function in the thermodynamic limit.
\begin{figure}[hbt]
    \centering
...
\begin{figure}[hbt]
    ...
    \caption{Example of the escape probability for $\lambda = 0.03$. Dots represent the results of the simulations over many samples, whereas the corresponding lines are the results of the fitting procedure.}
\end{figure}
\begin{figure}
    \centering
