...
\begin{equation}\label{eq:numsim:mhm:energyvariation}
    \Delta E =  -\frac{1}{\lambda} \left[ \log \left( \frac{\sum_{\mu = 1}^{M} \exp\left(\lambda \cdot m_{in}^\mu \right)\exp\left(-2\lambda\xi_{k}^{\mu}\sigma_k \right) }{\sum_{\mu = 1}^{M} \exp\left(\lambda \cdot m_{in}^\mu\right)} \right) \right].
\end{equation}
