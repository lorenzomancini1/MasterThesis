...
\begin{algorithm}
    ...
    \Require $\symbf{\sigma}$, $J$, $\beta$
        ...
        \EndIf
        \EndFor
    \textbf{return} $\symbf{\sigma}$, $\text{fliprate} / N$
    \end{algorithmic}
\end{algorithm}
...
We store $M = N \alpha$ patterns of length $N = 1000$. A random pattern is taken and perturbed with spin-flip probability $p = 0.2$ and the \acrlong{mcmc} is executed on the perturbed configuration for $\texttt{nsweeps} = 100$ with $\beta = 100$. Then the overlap between the retrieved state and the original one is measured. In \cref{fig:shm:p02:final_overlaps} it can be clearly observed that there is a sharp peak centered at very high magnetization \ie $m \approx 1$. However, it is worth to notice that also a broad distribution centered at a lower value of $m$ appears.  
\begin{figure}
    \centering
...
As can be observed, in \cref{fig:shm:p02:final_overlaps2} most of the weight of the distribution is contained around a small value of magnetization: there are too stored patterns and the model's performances on retrieval are bad.
...
\end{equation}
Here, the magnetization $m$ is not rescaled by $N$. (boh forse ho fatto un po' di confusione con la notazione).
We have two problems:
\begin{itemize}
    \item since in the \acrshort{mhm} the storage capacity increases exponentially, \cref{eq:numsim:mhm:energyvariation} involves a sum of a number of terms that is exponential;
...
Overflow in the exponential is the other potential numerical problem. However, this can be simply solved with the usual trick of subtracting the maximum from the exponential argument. In particular, we deal with exponential of the magnetiazions between a configuration and the stored patterns, \ie, $\exp\left(\symbf{\sigma}\cdot \symbf{\xi}^{\mu}\right)$ where $\mu = 1,\dots, M$. Writing
\begin{equation*}
...
In~\cref{alg:mhm_metropolis} we show the Metropolis version for the \acrlong{mhm}. The full \acrshort{mcmc} is the same as~\cref{alg:shm_mcmc}.
...
\begin{algorithm}
    ...
    \Require $\symbf{\sigma}$, $\symbf{\xi}$, $\beta$, $\lambda$
        ...
        \For{$i$ in random permutation $N$}
        \State $\sigma[i] \gets - \sigma[i]$
        \State $E_{new} \gets energy\left(\symbf{\sigma}, \symbf{\xi}, \lambda\right)$
...
The same considerations for the numerical stability argument hold also for the continuous case of the \acrshort{mhm}.\\
...
\begin{equation*}
    \tilde{\xi}_i^{\mu} = \xi_i^{\mu} + \mathcal{N}\left(0, \delta\right),
\end{equation*}
where $\tilde{\xi}_i^{\mu}$ is the perturbed version of the $i$-th element belonging to the $\mu$-th pattern.\\
The key operation of the update rule is the $\operatorname{softmax}$ operation, as stated in~\cref{eq:hop:cont_energy}. In~\cref{alg:softmax} the implementation of such function is shown~\cite{numerically_stable}.
...
\begin{algorithm}
    ...
    \State $\symbf{\sigma}_{rec} \gets \symbf{\xi} \times \operatorname{softmax}\left(\lambda \symbf{\xi}^T \times \symbf{\sigma}_{rec}\right)$
    \textbf{return} $\symbf{\sigma}_{rec}$
\end{algorithmic}
\end{algorithm}
